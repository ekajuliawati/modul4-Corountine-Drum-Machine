package com.example.corountinedrummachine
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        cymbal = MediaPlayer.create(this , R.raw.ring)
        tom = MediaPlayer.create(this , R.raw.ring2)
        btnStart.setOnClickListener {
            runBlocking {
                launch {
                    playBeats("x-x-x-x-x-x-x-x-x-x-x-x-" , R.raw.ring2)
                }
                playBeats("x-----x-----x-----x-----" , R.raw.ring)
            }
        }
    }
